...
#define LOOP  100///<初始化循环次数
#define Pse 0.9 ///SE的使用频率
#define INC 0.005 ///每次增加的初始感染比率
#define P_INFECT 0.01 ///通过搜索引擎感染的可能性
        ...
        for(float p=0;p<=1;p+=INC)
        {
            ...
            while(loop > 0)
            {
                ...
                for(; i < numberOfEdge; i++)
                {
                    if(arr1[Edges[i].u] == 1)
                    {
                        p2 = infectedProbabitity(P_INFECT);
                        if(p2 == 1)
                            arr2[Edges[i].v] = 1;
                    }
                    if(arr1[Edges[i].v] == 1)
